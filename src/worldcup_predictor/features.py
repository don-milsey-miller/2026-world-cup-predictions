...
def build_training_frame(matches: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, TeamState]]:
    ...
    for match in matches.sort_values("date").itertuples(index=False):
        ...
        update_states(home_state, away_state, int(match.home_score), int(match.away_score), match.date)
...
def matchup_features(
    *,
    ...
    return {
        ...
        "goals_against_diff": _mean(state_a.recent_against, 1.0) - _mean(state_b.recent_against, 1.0),
